...
class BinaryHeap {
public:
    // constructor not required because the only "initialization" is through the
    // constructor of the variable "heap" which is called by default

    // return the minimum element in the heap
    HeapItem<T, K> min() const;

    // insert an item with the given key
    // if the item is already in the heap, it  will still insert a new copy with
    // this key
...
void BinaryHeap<T, K>::fixHeapDown(int i) {
    while (true) {
        ...
        uint lchild = (i<<1)+1, rchild = (i<<1)+2;
