...
void process_input() {
    ...
    if (delta.x != 0 || delta.y != 0) {
        ...
        erase_cursor();
        // move the cursor, and the map view if the edge was nudged
        move_cursor(delta);
        if (shared.redraw_map == 0) {
            // draw the new cursor position after scrolling
            draw_cursor();
        }
    }
}
...
int main() {
    ...
    while (true) {
        ...
        if (shared.redraw_map) {
            ...
            if (curr_mode == WAIT_FOR_START) status_message("FROM?");
            else status_message("TO?");
